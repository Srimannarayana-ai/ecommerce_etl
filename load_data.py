...
def load_data_to_database(csv_filename: str, db_filename: str, table_name: str):
    ...
    """
    logging.info(f"Initiating load process from {csv_filename}...")
    
    df = pd.read_csv(csv_filename)
    
    logging.info(f"Establishing connection to target database: {db_filename}...")
    conn = sqlite3.connect(db_filename)
    
    logging.info(f"Inserting records into table '{table_name}'...")
